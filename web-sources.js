(() => {
  ...
  const CACHE_KEY = 'silicon-web-parts-v3';
  const CACHE_AGE = 24 * 60 * 60 * 1000;
  const HF_DATASET = 'Doshiba/pcpartpicker-parts-dataset';
  const webCategories = ['cpu','cpu-cooler','motherboard','memory','internal-hard-drive','video-card','power-supply','case'];
  const categoryMap = {
  ...
  function normalizeRow(row, index) {
    ...
    if (type === 'cpu') {
      const architecture = specValue(s,'Microarchitecture');
      p.socket = normalizeSocket(specValue(s,'Socket')) || inferCpuSocket(p.name,architecture);
  ...
  async function loadCategory(category, length) {
    const where = `"category"='${category}'`;
    const url = `https://datasets-server.huggingface.co/filter?dataset=${encodeURIComponent(HF_DATASET)}&config=default&split=train&where=${encodeURIComponent(where)}&offset=0&length=${Math.min(100,length)}`;
  ...
  async function loadDataset(limit) {
    const each = Math.max(1,Math.min(100,Math.ceil(limit/webCategories.length)));
    const settled = await Promise.allSettled(webCategories.map(category => loadCategory(category,each)));
    const rows=[];
    const failed=[];
    settled.forEach((result,index) => {
      if (result.status === 'fulfilled') rows.push(...result.value);
      else failed.push(webCategories[index]);
    });
    const normalized = rows.map((row,index)=>normalizeRow(row,index)).filter(Boolean).slice(0,limit);
    if (!normalized.length) throw new Error('No web parts were returned');
    normalized.failedCategories=failed;
    return normalized;
  }
  ...
  async function addWikimediaFallbacks(items) {
    ...
    await Promise.all(targets.map(async p => {
      try {
        p.image = await wikimediaImage(p.name);
        if (p.image) p.imageSource='Wikimedia Commons';
      } catch {}
    }));
  ...
  async function syncWebParts(auto=false) {
    ...
    try {
      ...
      out.innerHTML += `<div class="source-ok">✓ PC parts dataset: ${incoming.length} balanced parts</div>`;
      ...
      $('#tip').textContent='Web parts loaded across every available category. Compatibility checks use sockets, RAM generation, size, cooling, storage connections, and PSU wattage.';
    } catch(err) {
      out.innerHTML=`<div class="source-bad">✕ Web sync failed: ${err.message}</div>`;
      status.textContent='Sync failed';
      if(auto)$('#tip').textContent='The built-in catalog loaded, but the live web catalog could not be reached. Tap the web catalog button to retry.';
    } finally {
      syncing=false;
      btn.disabled=false;
